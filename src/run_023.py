#!/usr/bin/env python3
"""
run_023: OOF-optimized blend of run_012 (LGBM, full embed features) and
run_019 (PCA embed-only + LGBM, public SOTA 0.7520).
...
def _blend_oof_full(
    ...
    return _blend_test(p_a, p_b, alpha, space)
...
def main() -> None:
    ...
    p12_oof = np.load(ROOT / "runs" / RUN_012_ID / "oof_proba.npy").astype(np.float64)
    p19_oof = np.load(ROOT / "runs" / RUN_019_ID / "oof_proba.npy").astype(np.float64)
    p12_test = np.load(ROOT / "runs" / RUN_012_ID / "test_proba.npy").astype(np.float64)
    p19_test = np.load(ROOT / "runs" / RUN_019_ID / "test_proba.npy").astype(np.float64)

    f12 = _oof_f1(p12_oof, y)
    f19 = _oof_f1(p19_oof, y)
    print(f"run_012 solo OOF: {f12:.4f}")
    print(f"run_019 solo OOF: {f19:.4f}")

    print("\n" + "═" * 60)
    print("PAIRWISE BLEND: run_012 + run_019")
    print("═" * 60)
    r = pairwise_diagnostic(RUN_012_ID, p12_oof, RUN_019_ID, p19_oof, y)

    best_score = r["best_score"]
    best_alpha = r["best_alpha"]
    best_space = r["best_space"]
    gain_vs_012 = best_score - f12
    gain_vs_019 = best_score - f19

    print("\n" + "═" * 60)
    print("SUMMARY")
    print("═" * 60)
    print(f"  Best blend OOF:     {best_score:.4f}  ({best_space}, α={best_alpha:.3f} on {RUN_012_ID})")
    print(f"  Gain vs run_012:    {gain_vs_012:+.4f}")
    print(f"  Gain vs run_019:    {gain_vs_019:+.4f}")
    print(f"  Noise threshold:    {THRESHOLD:.4f}  "
          f"{'CLEARS ✓' if best_score > THRESHOLD else 'below — still may beat public SOTA'}")

    fold_scores = _per_fold_scores(p12_oof, p19_oof, y, best_alpha, best_space)
    ...
    oof_blend = _blend_oof_full(p12_oof, p19_oof, best_alpha, best_space).astype(np.float32)
    test_blend = _blend_test(p12_test, p19_test, best_alpha, best_space).astype(np.float32)

    rm = RunManager()
    run_id = "run_023"
    run_dir = rm.run_path(run_id)
    blend_params = {
        "base_runs": [RUN_012_ID, RUN_019_ID],
        "alpha_run_012": best_alpha,
        "weight_run_019": 1.0 - best_alpha,
        "blend_space": best_space,
        "solo_oof": {RUN_012_ID: f12, RUN_019_ID: f19},
    }
    if not run_dir.exists():
        rm.create_run(
            description="OOF blend run_012 LGBM + run_019 PCA-embed LGBM",
            ...
            cv_folds=CV_FOLDS,
            cv_metric="micro_f1",
            notes=f"Tier A blend. Gain vs run_012 {gain_vs_012:+.4f}, vs run_019 {gain_vs_019:+.4f}.",
        )
    else:
    ...
    meta["blend_alpha_run_012"] = best_alpha
    meta["solo_oof_run_012"] = f12
    meta["solo_oof_run_019"] = f19
    meta["gain_vs_run_012"] = gain_vs_012
