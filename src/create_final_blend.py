#!/usr/bin/env python3
"""
create_final_blend.py
Generates the final Kaggle submission by blending run_026 (SOTA) and run_042 (LOO Spatial).
...
def main():
    ...
    p42 = np.load(ROOT / "runs" / "run_042" / "test_proba.npy")
    
    # Convert to logits
    p26_logits = np.log(np.clip(p26, 1e-15, 1 - 1e-15))
    p42_logits = np.log(np.clip(p42, 1e-15, 1 - 1e-15))
    
    # Optimal logit blend weights from pairwise_diagnostic
    # alpha=0.227 on run_026
    alpha = 0.227
    print(f"Blending Logits: {alpha} * run_026 + {1 - alpha} * run_042")
    
    blend_logits = (alpha * p26_logits) + ((1 - alpha) * p42_logits)
    ...
    out_path = ROOT / "outputs" / "submission_final_blend_run042_run026.csv"
